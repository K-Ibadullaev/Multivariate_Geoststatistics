...
```
 We are after cobalt ores which are often occurs within Ni ore deposits


```{r Descriptive summaries2}
...
## Bivariate analysis
 Co vs Ni  A scatterplot of these two variables of interest.
...
### Data maps
 Bubble and color maps are shown for each variable of interest. The following plots represent the log-concentration of variables Co and Ni.
```{r Data maps 1}
# Co
...
```
 One can observe that the highest values of log(Co) and Log(Ni) are located in North and the North-West regions.

 The Log(Co) have larger variability in comparison to Log(Ni). One can see that
lowest values are distributed in direction from SW to NE and prevail in the center. Also the locations of the largest values overlap each other.
Thus, there's some relationship between variables. One can also notice that higher of Ni prevail over Co as it usually occurs in the real life.
...
# Co as a function of location (one coordinate) : swath plot
...
```
 Reasonable to assume the stationarity for log(Ni) and log(Co) as the curve is smooth doesn't change significantly.
 
### Structural analysis
## Marginal variograms
###  Numeric variables (isotropic)
One starts with modeling of isotropical variograms for both variables Co and Ni
with regard to local neigbourhood.
```{r Isotropic variography Co}
#Co
...
vgo_Co = variogram(gs_Co, cutoff=225, width=11.25)  
plot(vgo_Co)
...
vgm_params
```
The next step is to expand the grid and to do kriging.
```{r Local kriging iso Co and Ni}
...
```
One can also convey the same procedure for the global kriging. But further it'd be clear,that in case of this dataset the difference isn't significant.
```{r global varms and kriging}
# global
...
```
Finally, the results have to visualized in order to compare them.
```{r iso local and global kriging}
# plot
...
```
Visually it's obvious, that global results are smoother than local.That can lead
to missing the local peculiarities of concentration. So the local kriging is more suitable in this case.


### Anisotropic variograms ----------------
Now we check the anisotropy.
...
# analyzing anisotrpic variogram one can say that the angle of anisotropy is 18°
# with ratio r=0.5
...
that both Co and Ni have similar anisotropy ratios and the same angle, which could be the indicator of their codependency. The following plots are the visualisation of results of kriging for aforementioned anisotropical variograms.
...
```
One can observe that the anisotropy has almost no impact on results of
kriging. So it's appropriate to consider the isotropy.
The next step is a simulation of kriging, a group realisations would be produced.
```{r univar sims}
#ggplot version
...
### Categorical variables(isotropic)
Now we explore impact of the given lithology .Additionally to the chemical concentrations, the data set includes as well information about the lithology of the underlying bedrock, in the variable Lcode
```{r catvar}
par(mfrow=c(1,1))
...
times less frequent than FZ.Now we inspect the behavior and influence on chemical variables log(Co) and log(Ni) using boxplots
```{r catboxplots}
par(mfrow=c(1,2))
...
```
 ...
 show lower concentrations of these variables. Furthermore, FZ has a large   variability and many outliers, UM has the lowest occurrence in the data set.
 Thus one can assume that SA and SM associated with CO and Ni.
 Therefore we carry on with the assumption of an isotropy and consider SA and SM related to Co and Ni. There's a dependence which would be inspected in sections below.


## MULTIVARIATE
### Numerical variables
Initially, it's common to start with the exploration of
the interaction between all chemical variables in the data set.
```{r multivarchem}
...
plot(vge_all)
## variogram model ---------------------------
# vgt_all = vgm(psill=0.2, range=70, nugget=0.08, model="Sph")
vgt_all = vgm(psill=0.2, range=70, nugget=0.08, model="Exp", add.to=
            vgm(psill=0.05, range=80, model="Sph"))
gs_chem_all = gstat::fit.lmc(v=vge_all, g = gs_chem_all, model = vgt_all, correct.diagonal = 1.01)   
...
logFillermap=ggplot()+
  ...
  theme()+
  scico::scale_fill_scico(palette = "batlowW")
gridExtra::grid.arrange(logAlmap, logComap,logFemap,logFillermap,logMgdmap,logNidmap,nrow=3,ncol=2) 




```
The result is the visualization of kriging for each variable. The most interesting are Co(1st row, 2nd col) and Ni(3rd row,2nd col) which again show their higher values at the similar locations as in univariate case, which is one more evidence of their codependency.


```{r multivar simchems}
#simulation 
cosim_l = predict(gs_chem_all, newdata = xy_grid, debug.level = -1, nsim=20)#100
#ggplot version
allsimulationplot = function(i,sim_ds,n){
  a=ggplot()+
  ...
  b=ggplot()+
    geom_raster(data=sim_ds ,aes( x=X, y=Y,fill=sim_ds[,2+i+1*n]))+
    coord_fixed()+
    theme_classic()+
    scico::scale_fill_scico(palette = "vik")+
    ggtitle(paste(sep="",i," Realisation of log  Al" ))
  c=ggplot()+
    geom_raster(data=sim_ds,aes( x=X, y=Y,fill=sim_ds[,2+i+2*n]))+
    coord_fixed()+
    theme_classic()+
    scico::scale_fill_scico(palette = "lajolla")+
    ggtitle(paste(sep="",i," Realisation of log Fe " ))
    d=ggplot()+
    geom_raster(data=sim_ds,aes(x=X, y=Y,fill=sim_ds[,2+i+4*n]))+
    coord_fixed()+
    theme_classic()+
    scico::scale_fill_scico(palette = "imola")+
  ggtitle(paste(sep="",i," Realisation of log Mg " ))
  e=ggplot()+
    geom_raster(data=sim_ds ,aes(x=X, y=Y,fill=sim_ds[,2+i+5*n]))+
...
In the section below presented one more common method of multivariate analysis
known as  compositional data analysis(CoDa). By the means of CoDa one can obtain
another look on multivariate case, especially it displays an anisotropy more vividly.
Also it's possible to represent the coupled data(here FE,Co,Ni) as tertiary diagram.
Which depicts the concentration of each chemical variable.
...
gmv = make.gmCompositionalGaussianSpatialModel(
  ...
  omax = 15,
  #define to boost the calculation of cokriging
  maxdist=100#define to boost the calculation of cokriging
...
lrvg = gmGeostats::logratioVariogram(gmv,maxdist=600,nbins=10)
plot(lrvg)
...
all_lcode_simulationplot(i=15,simlcodes)
...
The most probable area of SM overlaps the regions of Co and Ni or even coincides with the regions of the high values of Co and Ni.

### Mixed variograms. Categorical + numeric variables.
Now it's time to consider both types of variables. The model for both Co and Ni incorporated with lithology types is computed below. 
```{r lcode +Co/Ni1}
## Lcode + Co
...
The conclusions  following from the summary statistics are :
 ...
 Now one considers Lcode-Co simultaneously
...
Now the similar procedure is done for Ni.
...
P.S. Extra section.
It's also possible to make several simulations for both Co and Ni but it's computationally expensive. This procedure takes several hours even for 1 simulation on my laptop, so I won't run the following code below. More powerful CPU is needed. 
.

```{r EXTRA sims}
# cok_Co4lcodessim = predict(gs_Co4lcodes, newdata=xxyy_grid, debug.level = -1, nsim=100)
# 
# cok_Ni4lcodessim = predict(gs_Ni4lcodes, newdata=xxyy_grid, debug.level = -1, nsim=100)


```

## Conclusions:
The main goal of this report is to  examine relationship between Co and Ni and one of lithological type (SM) considering spatial distribution.
These assumptions of codependency  are proved by the means of several techniques with regard to number and types of examined variables: Univariate, Bivariate and Multivariate.The regions of their highest values of Co and Ni, as well as probability of SM overlap and occur at the almost the same locations through all stages of analysis. The results are quite consistent.  The dependency between the lithological type SM and Co and Ni is found and does exist.
Using the indicator kriging the interpolation for categorical variables is obtained. Though it is not the main objective, it supports the examined assumptions and doesn't contradict them at all. 

Furthermore, it's reasonable to assume that the given region is an ore deposit of pyrite. Namely,Cobalt-bearing  Cattierite,and Nickel-bearing Bravoite.
One of tertiary diagrams in CoDa section exposes the percentage of the concentration for Fe, Co and Ni.  
